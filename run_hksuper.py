...
try:
    ...
    print("⚠️ 未偵測到 futu-api (如果您在雲端運行，此為正常現象，將自動關閉期權驗證)。")

# ==========================================
# 1. 系統配置中心 (V115.1 Bug修復版)
# ==========================================
...
GURU_LIST_HK = [
    ...
    "0669.HK", "0151.HK", "9992.HK", "9633.HK", "0867.HK", "0316.HK", "1997.HK",
...
def check_futu_options(ctx, yf_ticker):
    ...
    try:
        ...
        if ret == RET_OK and not snapshots.empty:
            ...
            if call_vols + put_vols == 0: return "期權零成交", 0
            if put_vols == 0: put_vols = 1 
            
            call_ratio = call_vols / put_vols
            
            if call_ratio > 2.5: return f"🔥大單暴買(C/P {call_ratio:.1f})", 25 
            elif call_ratio > 1.5: return f"📈多單偏多(C/P {call_ratio:.1f})", 15 
            elif call_ratio < 0.5: return f"🚨空單湧入(C/P {call_ratio:.1f})", -25 
            else: return f"⚖️多空平衡(C/P {call_ratio:.1f})", 0
...
def run_super_growth_hk_v115():
    ...
    print("\n" + "="*60)
    print(f"🚀 [港股 動量矩陣 V115.1] 啟動 | Bug修復與雲端適配版")

    futu_ctx = None
    if FUTU_AVAILABLE:
        try:
            futu_ctx = OpenQuoteContext(host='127.0.0.1', port=11111)
            print("🟢 成功連接本地 FutuOpenD！")
        except Exception as e:
            print("🔴 無法連接 FutuOpenD (可能是雲端環境)，期權透視將自動關閉。")
    ...
    matrix = [[f"Momentum Portfolio V115.1 (雲端相容版)", f"{update_time}", ""] + [""] * 9, headers_col]
