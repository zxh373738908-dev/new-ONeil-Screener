...
def sync_to_google_sheet(sheet_name, matrix):
    try:
        ...
        print(f"🎉 V96 獲利狂奔版 同步完成！持續為您雷達鎖定。")
...
# ==========================================
# 3. 核心量化模型 V96 (Let Profits Run)
# ==========================================
def run_super_growth_v96():
    update_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    universe = get_universe()
    
    print("\n" + "="*50)
    print(f"🚀 [超級成長股 V96] 啟動 | 陣地戰模式，讓利潤狂奔...")
    ...
    try:
        ...
        weather = "☀️ 穩健攀升" if curr_spy > ma50_spy and vix_val < 22 else ("☁️ 震盪洗盤" if curr_spy > ma50_spy else "📉 跌破趨勢")
        strategy = "🛡️ 陣地戰：死抱強勢，讓利潤狂奔" if vix_val < 20 else "⚠️ 提高警覺，緊盯停損"
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        for t, info in executor.map(fetch_info_v96, list(tech_results.keys())):
    ...
    m_info = f"{weather} | 寬度:{us_breadth:.1f}% | VIX:{round(vix_val, 1)} | {strategy} | {macro_text}"
    
    matrix = [[f"Master Sniper V96 (Let Profits Run)", f"更新: {update_time}", m_info] + [""] * (len(headers) - 3), headers]
