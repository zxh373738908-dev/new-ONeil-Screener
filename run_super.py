...
def get_universe():
    # 💡 核心進化：植入大師最新持倉與重點跟蹤名單
    core = ["DUOL", "FSLR", "LLY", "NDAQ", "NTNX", "OKTA", "PWR", "ROKU", "V", "VRT", 
            "CAVA", "FIVE", "HWM", "MPWR", "LITE", "MU", "SNDK", "GEV", "ALB", "SMCI"]
    try:
...
def run_super_growth_v82():
    ...
    # 1. 宏觀與天氣
    try:
        m_data = yf.download(["SPY", "^VIX", "BNO", "GLD", "CPER"], start="2024-09-01", progress=False)['Close'].dropna()
        spy_hist = m_data['SPY']
        ...
        strategy = "全面進攻" if curr_spy > ma50_spy and vix_val < 18 else ("板塊輪動(防禦)" if curr_spy > ma50_spy else "戰略撤退")
    ...
    # 2. 技術面深潛掃描
    print(f"📡 正在掃描 {len(universe)} 隻標的 (尋找 20EMA 黃金坑)...")
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        for t, info in executor.map(fetch_info_v82, list(tech_results.keys())):
    ...
    for t, data in tech_results.items():
        ...
        msg = f"R40({round(rev_g + fcf_m)}%)" if 'Tech' in sec or 'Software' in ind else f"利潤({round(info.get('operatingMargins', 0)*100, 1)}%)"
        if data['VolRatio'] > 1.3: msg += f" | 📈爆量"

        all_cands.append({
            "T": t, "Sec": sec, "Ind": ind[:16], "Score": score, "Action": action, "Msg": msg,
            "YTD": data['YTD'], "Trend": data['Trend'], "RS": rs, "Rate": rating,
